...
public class ModeSettingComponent extends SettingComponent {
    ...
    public void render(MatrixStack var1, int var2, int var3, float var4) {
        ...
        for (String var11 : this.value.getValues()) {
            ...
            float pillY = this.position.y + this.margin + (this.margin / 2) + var5 + var7 + this.margin / 2.0F;
            float pillW = Fonts.MONTSERRAT_MEDIUM.getWidth(var11, fontSize) + var9 * 2.0F;
            float pillH = 7F + var9 * 2.0F;
            float selVal = (float) var12.get();
            if (selVal > 0.01f) {
            ...
            Fonts.MONTSERRAT_MEDIUM.draw(var1, var11, this.position.x + this.margin + var6, pillY + var9 + 0.1F, textCol, fontSize);
            var6 += var13;
        }
