...
import dev.wh1tew1ndows.client.utils.render.shader.impl.util.BloomShader1;
import dev.wh1tew1ndows.client.utils.render.shader.impl.util.BloomShader2;
...
public class BloomShader implements IMinecraft {
    ...
    private final ShaderManager upSample = ShaderManager.create(new BloomShader1());
    private final ShaderManager downSample = ShaderManager.create(new BloomShader2());
    public List<IRenderCall> displayBlurQueue = new LinkedList<>();
    public List<IRenderCall> cameraBlurQueue = new LinkedList<>();
    private Framebuffer inFramebuffer = new Framebuffer(mc.getMainWindow().getFramebufferWidth(), mc.getMainWindow().getFramebufferHeight(), true, false);
    private Framebuffer outFramebuffer = new Framebuffer(mc.getMainWindow().getFramebufferWidth(), mc.getMainWindow().getFramebufferHeight(), true, false);
    ...
    private void initFramebuffers(float iterations) {
        ...
        framebufferList.add(outFramebuffer = ShaderManager.createFrameBuffer(null, false));
        ...
        for (int i = 0; i <= iterations; i++) {
            ...
            Framebuffer currentBuffer = new Framebuffer(width, height, false, false);
    ...
    public void draw(int iterations, float offset, RenderType type, float saturation) {
        ...
        if (inFramebuffer.framebufferWidth != mc.getMainWindow().getFramebufferWidth() || inFramebuffer.framebufferHeight != mc.getMainWindow().getFramebufferHeight()) {
            ...
            inFramebuffer = ShaderManager.createFrameBuffer(inFramebuffer);
        }
        ...
        switch (type) {
            ...
            case DISPLAY -> {
                ...
                RenderUtil.GLU.startBlend();
                drawQuads();
                RenderUtil.GLU.endBlend();
    ...
    private void renderFBO(Framebuffer framebuffer, int framebufferTexture, ShaderManager shader, float offset) {
