...
public class Font implements IMinecraft {
    ...
    public static Font create(String file, float size) {
        ...
        try {
            InputStream in = Preconditions.checkNotNull(Font.class.getResourceAsStream("/assets/minecraft/" + "zetrix" + "/fonts/ttf/" + file), "Font resource is null");
    ...
    public void drawGradient(MatrixStack matrix, String text, double x, double y, int firstColor, int secondColor) {
        int[] colors = RenderUtil.Colors.genGradientForText(firstColor, secondColor, text.length());
        float xOffset = 0;
    ...
    public void drawCenter(String text, float x, float y, int color) {
        draw(new MatrixStack(), text, x - getWidth(text) / 2f, y, color, false);
    }
    ...
    public void drawOutline(MatrixStack matrix, String text, double x, double y, int color, float multDark) {
        float outline = 0.5F;
        int alpha = RenderUtil.Colors.alpha(color);
...
//            }
    ...
    private void renderStringAtPos(MatrixStack matrixStack, String text, boolean hasShadow, int color) {
        ...
        for (int i = 0; i < text.length(); ++i) {
            ...
            if (character == 167 && i + 2 < text.length()) {
                if (text.charAt(i + 1) == '#' && i + 8 < text.length()) {
                    ...
                    if (formatting != null && formatting.getColor() != null) {
                        int colorCode = formatting.getColor();
                        red = (float) (colorCode >> 16 & 255) / 255.0F;
                        green = (float) (colorCode >> 8 & 255) / 255.0F;
                        blue = (float) (colorCode & 255) / 255.0F;
                    } else {
