...
public final class RotationUtils {
    ...
    public static Rotation smoothRotate(Rotation current, Rotation target) {

        float currentYaw = Rotation.normalizeYaw(current.getYaw());
        float targetYaw = Rotation.normalizeYaw(target.getYaw());
        float deltaYaw = Rotation.normalizeYaw(targetYaw - currentYaw);

        float currentPitch = current.getPitch();
        float targetPitch = Rotation.clampPitch(target.getPitch());
        float deltaPitch = targetPitch - currentPitch;

        float yawSpeed = 2.5f + Math.min(Math.abs(deltaYaw) / 45f, 4.0f);
        float pitchSpeed = 2.0f + Math.min(Math.abs(deltaPitch) / 20f, 3.0f);
        ...
        yawStep += ThreadLocalRandom.current().nextFloat(-0.05f, 0.05f);
        pitchStep += ThreadLocalRandom.current().nextFloat(-0.05f, 0.05f);

        float newYaw = GCDUtil.getSensitivity(currentYaw + yawStep);
        float newPitch = GCDUtil.getSensitivity(Rotation.clampPitch(currentPitch + pitchStep));
