...
public class ConnectedTextures {
    ...
    private static boolean skipConnectedTexture(IBlockReader blockAccess, BlockState blockState, BlockPos blockPos, BakedQuad quad, RenderEnv renderEnv) {
        ...
        if (block instanceof PaneBlock) {
            ...
            if (d1 < 0.4D) {
                return blockstate.get(FourWayBlock.WEST);
            } else if (d1 > 0.6D) {
                return blockstate.get(FourWayBlock.EAST);
            } else {
                double d0 = quad.getMidZ();

                if (d0 < 0.4D) {
                    return blockstate.get(FourWayBlock.NORTH);
                } else {
                    if (!(d0 > 0.6D)) {
                        return true;
                    }

                    return blockstate.get(FourWayBlock.SOUTH);
                }
            }
        }
        ...
        if (ibakedmodel == null) {
            ...
            if (list == null) {
                ...
                if (list.size() > 0) {
                    ...
                    List list1 = ibakedmodel.getQuads(neighbourState, null, RANDOM);
