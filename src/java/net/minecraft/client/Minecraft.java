...
import dev.wh1tew1ndows.baritone.api.event.events.WorldEvent;
import dev.wh1tew1ndows.baritone.api.event.events.type.EventState;
...
import dev.wh1tew1ndows.client.managers.events.other.ScreenResizeEvent;
...
import dev.wh1tew1ndows.common.impl.viaversion.fixes.AttackOrder;
...
public class Minecraft extends RecursiveEventLoop<Runnable> implements IWindowEventListener {
    ...
    public static boolean isScreenMinimized() {
        return getInstance().getMainWindow().getWidth() == 0 && getInstance().getMainWindow().getHeight() == 0;
    }
    ...
    public void displayScreen(@Nullable Screen guiScreenIn) {
        ...
        if (guiScreenIn instanceof MainMenu || guiScreenIn instanceof MultiplayerScreen) {
            ...
            if (!(Zetrix.initialized() && BetterMinecraft.getInstance().isEnabled() && BetterMinecraft.getInstance().keepChat.getValue()))
    ...
    public void runTick() {
        ...
        if (this.currentScreen == null && this.player != null) {
            BiFunction<EventState, dev.wh1tew1ndows.baritone.api.event.events.TickEvent.Type, dev.wh1tew1ndows.baritone.api.event.events.TickEvent> tickProvider = dev.wh1tew1ndows.baritone.api.event.events.TickEvent.createNextProvider();
            BaritoneAPI.getProvider().getAllBaritones().forEach(baritone -> {
                dev.wh1tew1ndows.baritone.api.event.events.TickEvent.Type type = baritone.getPlayerContext().player() != null && baritone.getPlayerContext().world() != null
                        ? dev.wh1tew1ndows.baritone.api.event.events.TickEvent.Type.IN
                        : dev.wh1tew1ndows.baritone.api.event.events.TickEvent.Type.OUT;
        ...
        if (this.loadingGui == null && (this.currentScreen == null || (BaritoneAPI.getProvider().getPrimaryBaritone().getPathingBehavior().isPathing() && player != null) || currentScreen.passEvents)) {
    ...
    public boolean isEntityGlowing(Entity entity) {
        return entity.isGlowing() || this.player != null && this.player.isSpectator() && this.gameSettings.keyBindSpectatorOutlines.isKeyDown() && entity.getType() == EntityType.PLAYER && !Zetrix.inst().moduleManager().get(NoRender.class).elements().getValue("Эффект свечения");
    }
    ...
    public static void sendBreakblocktonuker(BlockPos block) {
        ...
        Direction closestDirection = calculateClosestDirection(playerPos, blockCenter);
        if (!world.getBlockState(block).isAir()) {
            if (minecraft.playerController.onPlayerDamageBlock(block, closestDirection)) {
                minecraft.particles.addBlockHitEffects(block, closestDirection);
                player.swingArm(Hand.MAIN_HAND);
            }
        }
    }

    private static Direction calculateClosestDirection(Vector3d playerPos, Vector3d blockCenter) {
        double deltaX = blockCenter.getX() - playerPos.getX();
        double deltaY = blockCenter.getY() - playerPos.getY();
        double deltaZ = blockCenter.getZ() - playerPos.getZ();
