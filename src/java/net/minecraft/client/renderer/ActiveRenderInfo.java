...
public class ActiveRenderInfo {
    ...
    public void update(IBlockReader worldIn, Entity renderViewEntity, boolean thirdPersonIn, boolean thirdPersonReverseIn, float partialTicks) {
        ...
        double zoomDistance = CustomCamera.getInstance().isEnabled() ? CustomCamera.getInstance().sliderSetting.getValue() : 4F;
        if (thirdPersonIn) {
            if (thirdPersonReverseIn) {
                this.setDirection(event.getYaw() + 180.0F, -event.getPitch());
            }

            if (CustomCamera.getInstance().isEnabled()) {
                this.movePosition(-this.calcCameraDistance(zoomDistance), 0.0D, CustomCamera.getInstance().sliderSetting2.getValue());
            } else {
