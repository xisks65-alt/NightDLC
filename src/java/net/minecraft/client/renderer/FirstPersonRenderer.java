...
public class FirstPersonRenderer {
    ...
    private void transformFirstPerson(MatrixStack matrixStackIn, HandSide handIn, float swingProgress) {
        CustomHand viewModel = CustomHand.getInstance();
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(handIn)) {
            ...
            if (stack.isEmpty()) {
                ...
                if (player.isHandActive() && player.getItemInUseCount() > 0 && player.getActiveHand() == handIn) {
                    ...
                    CustomHand swingAnimations = CustomHand.getInstance();


                    if (swingAnimations.swingMode.is("Дефолтная") && swingAnimations.auraCheck() && handside == this.mc.gameSettings.mainHand && swingAnimations.isEnabled()) {
                        float f = MathHelper.sin(swingProgress * swingProgress * (float) Math.PI);
                        matrixStackIn.translate(handOffset * 0.56F, -0.5f, -0.72F);
                        matrixStackIn.rotate(Vector3f.YP.rotationDegrees(handOffset * (45.0F + f * -20.0F)));
                        float f1 = MathHelper.sin(MathHelper.sqrt(swingProgress) * (float) Math.PI);
                        matrixStackIn.rotate(Vector3f.ZP.rotationDegrees(handOffset * ((float) 1 * f1 * -20.0F)));
                        matrixStackIn.rotate(Vector3f.XP.rotationDegrees(f1 * -1.0F * swingAnimations.animgsize.getValue() * 10));
                        matrixStackIn.rotate(Vector3f.YP.rotationDegrees(handOffset * -45.0F));
                    } else {
