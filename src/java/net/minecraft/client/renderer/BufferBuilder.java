...
public class BufferBuilder extends DefaultColorVertexBuilder implements IVertexConsumer {
    ...
    private final MultiTextureBuilder multiTextureBuilder = new MultiTextureBuilder();
    ...
    private final int[] tempInt4 = new int[4];
    ...
    private final Vector3f midBlock = new Vector3f();
    ...
    public void nextVertexFormatIndex() {
        ...
        int newIndex = this.vertexFormatIndex;
        int elementSize = this.vertexFormatElement.getSize();

        do {
            newIndex = (newIndex + 1) % size;
            this.nextElementBytes += elementSize;
            VertexFormatElement element = elements.get(newIndex);
        ...
        this.vertexFormatIndex = newIndex;
    ...
    public void addVertex(float x, float y, float z, float red, float green, float blue, float alpha, float texU, float texV, int overlayUV, int lightmapUV, float normalX, float normalY, float normalZ) {
        if (this.defaultColor) {
            ...
            if (this.fastFormat) {
                ...
                this.putShort(i, (short) (lightmapUV & 65535));
