...
public class FaceBakery {
    private static final float SCALE_ROTATION_22_5 = 1.0F / (float) Math.cos((float) Math.PI / 8F) - 1.0F;
    private static final float SCALE_ROTATION_GENERAL = 1.0F / (float) Math.cos((float) Math.PI / 4F) - 1.0F;
    ...
    public static BlockFaceUV updateFaceUV(BlockFaceUV blockFaceUVIn, Direction facing, TransformationMatrix modelRotationIn, ResourceLocation modelLocationIn) {
        ...
        int i = Math.floorMod(-((int) Math.round(Math.toDegrees(Math.atan2(vector3f.getY(), vector3f.getX())) / 90.0D)) * 90, 360);
