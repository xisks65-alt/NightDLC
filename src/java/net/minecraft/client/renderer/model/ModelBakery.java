...
public class ModelBakery {
    ...
    public ModelBakery(IResourceManager resourceManagerIn, BlockColors blockColorsIn, int maxMipmapLevel) {
        this(resourceManagerIn, blockColorsIn, true);
        this.processLoading(maxMipmapLevel);
    }
    ...
    protected void processLoading(int p_processLoading_2_) {
        ...
        for (Entry<ResourceLocation, List<RenderMaterial>> entry : map.entrySet()) {
            ...
            AtlasTexture.SheetData atlastexture$sheetdata = atlastexture.stitch(this.resourceManager, entry.getValue().stream().map(RenderMaterial::getTextureLocation), p_processLoading_2_);
            this.sheetData.put(entry.getKey(), Pair.of(atlastexture, atlastexture$sheetdata));
        }

    }

    public SpriteMap uploadTextures(TextureManager resourceManagerIn) {
    ...
    static <T extends Comparable<T>> T parseValue(Property<T> property, String value) {
        return property.parseValue(value).orElse(null);
    }
                            ...
                            try {
                                immutablelist.stream().filter(parseVariantKey(statecontainer, p_lambda$loadBlockstate$23_9_)).forEach((p_lambda$null$22_6_) -> {
                                    ...
                                    if (pair2 != null && pair2.getFirst() != multipart) {
                                        ...
                                        throw new RuntimeException("Overlapping definition with: " + blockmodeldefinition.getVariants().entrySet().stream().filter((p_lambda$null$21_1_) ->
                                        {
    ...
    public IBakedModel getBakedModel(ResourceLocation p_getBakedModel_1_, IModelTransform p_getBakedModel_2_, Function<RenderMaterial, TextureAtlasSprite> p_getBakedModel_3_) {
        ...
        if (this.bakedModels.containsKey(triple)) {
            ...
            if (iunbakedmodel instanceof BlockModel blockmodel) {
    ...
    static class ModelListWrapper {
        ...
        public boolean equals(Object p_equals_1_) {
            if (this == p_equals_1_) {
                return true;
            } else if (!(p_equals_1_ instanceof ModelListWrapper modelbakery$modellistwrapper)) {
                return false;
            } else {
