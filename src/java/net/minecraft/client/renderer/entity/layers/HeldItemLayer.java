...
public class HeldItemLayer<T extends LivingEntity, M extends EntityModel<T> & IHasArm> extends LayerRenderer<T, M> {
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (CustomModel.getInstance().isEnabled() && CustomModel.getInstance().mode().is("Джефф") && (entitylivingbaseIn == mc.player || Zetrix.inst().friendManager().isFriend(entitylivingbaseIn.getName().getString()))) {
            ItemStack itemstack;
            boolean flag = entitylivingbaseIn.getPrimaryHand() == HandSide.LEFT;
