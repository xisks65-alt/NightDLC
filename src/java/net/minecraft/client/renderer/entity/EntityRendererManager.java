...
import dev.wh1tew1ndows.baritone.utils.accessor.IEntityRenderManager;
import dev.wh1tew1ndows.client.managers.events.other.EntityRenderMatrixEvent;
...
public class EntityRendererManager implements IEntityRenderManager {
    ...
    public <E extends Entity> void renderEntityStatic2(E entityIn, double xIn, double yIn, double zIn, float rotationYawIn, float partialTicks, boolean partialTicks2, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        if (this.info != null) {
            EntityRenderer<? super E> entityrenderer = this.getRenderer(entityIn);


            float partialTicks23 = 2;
            try {
                Vector3d vector3d = entityrenderer.getRenderOffset(entityIn, partialTicks);
                double d2 = xIn + vector3d.getX();
                double d3 = yIn + vector3d.getY();
