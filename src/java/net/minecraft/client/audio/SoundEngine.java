...
public class SoundEngine {
    ...
    public boolean isPlaying(ISound soundIn) {
        if (!this.loaded) {
            ...
            return this.playingSoundsStopTime.containsKey(soundIn) && this.playingSoundsStopTime.get(soundIn) <= this.ticks || this.playingSoundsChannel.containsKey(soundIn);
        }
    }
                        ...
                        if (f2 == 0.0F && !p_sound.canBeSilent()) {
                            ...
                            if (!this.listeners.isEmpty()) {
                                ...
                                if (flag1) {
                                    ...
                                    LOGGER.debug(LOG_MARKER, "Did not notify listeners of soundEvent: {}, it is too far away to hear", resourcelocation);
                                }
                            }
                            ...
                            if (this.listener.getGain() <= 0.0F) {
                                ...
                                if (channelmanager$entry == null) {
                                    LOGGER.warn("Failed to create new sound post");
                                } else {
                                    ...
                                    channelmanager$entry.runOnSoundExecutor((source) ->
                                    {
                                        ...
                                        if (isound$attenuationtype == ISound.AttenuationType.LINEAR) {
                                            source.setLinearAttenuation(finalF);
                                        } else {
