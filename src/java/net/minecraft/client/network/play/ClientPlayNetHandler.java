...
import dev.wh1tew1ndows.baritone.api.event.events.ChunkEvent;
import dev.wh1tew1ndows.baritone.api.event.events.type.EventState;
import dev.wh1tew1ndows.baritone.cache.CachedChunk;
import dev.wh1tew1ndows.client.managers.events.other.DeathEvent;
import dev.wh1tew1ndows.client.managers.events.other.TotemPopEvent;
...
public class ClientPlayNetHandler implements IClientPlayNetHandler {
    ...
    public void handleMultiBlockChange(SMultiBlockChangePacket packetIn) {
        ...
        BaritoneAPI.getProvider().getAllBaritones().forEach(baritone -> {
            ...
            if (player != null && player.connection == this) {
                baritone.getGameEventHandler().onChunkEvent(new ChunkEvent(EventState.POST, ChunkEvent.Type.POPULATE_FULL, chunkPos[0].x, chunkPos[0].z));
            }
        });
    }
    ...
    public void handleChunkData(SChunkDataPacket packetIn) {
        ...
        BaritoneAPI.getProvider().getAllBaritones().forEach(baritone -> {
            ClientPlayerEntity player = baritone.getPlayerContext().player();
            if (player != null && player.connection == this) {
                baritone.getGameEventHandler().onChunkEvent(new ChunkEvent(EventState.POST, packetIn.isFullChunk() ? ChunkEvent.Type.POPULATE_FULL : ChunkEvent.Type.POPULATE_PARTIAL, packetIn.getChunkX(), packetIn.getChunkZ()));
            }
        });
    }
    ...
    public void processChunkUnload(SUnloadChunkPacket packetIn) {
        BaritoneAPI.getProvider().getAllBaritones().forEach(baritone -> {
            ClientPlayerEntity player = baritone.getPlayerContext().player();
            if (player != null && player.connection == this) {
                baritone.getGameEventHandler().onChunkEvent(new ChunkEvent(EventState.PRE, ChunkEvent.Type.UNLOAD, packetIn.getX(), packetIn.getZ()));
            }
        });
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
        ...
        BaritoneAPI.getProvider().getAllBaritones().forEach(baritone -> {
            ClientPlayerEntity player = baritone.getPlayerContext().player();
            if (player != null && player.connection == this) {
                baritone.getGameEventHandler().onChunkEvent(new ChunkEvent(EventState.POST, ChunkEvent.Type.UNLOAD, packetIn.getX(), packetIn.getZ()));
            }
        });
    }
    ...
    public void handleBlockChange(SChangeBlockPacket packetIn) {
        ...
        BaritoneAPI.getProvider().getAllBaritones().forEach(baritone -> {
            ClientPlayerEntity player = baritone.getPlayerContext().player();
            if (player != null && player.connection == this) {
                baritone.getGameEventHandler().onChunkEvent(new ChunkEvent(EventState.POST, ChunkEvent.Type.POPULATE_FULL, packetIn.getPos().getX() >> 4, packetIn.getPos().getZ() >> 4));
            }
        });
    }
...
//            LOGGER.warn("Skipping Entity with id {}", packetIn.getEntityType());
    ...
    public void handleEntityStatus(SEntityStatusPacket packetIn) {
        ...
        if (packetIn.getOpCode() == 21) {
            ...
            Particles particlesModule = Particles.getInstance();
            if (!particlesModule.events().getValue("Тотем") && particlesModule.isEnabled()) {
            ...
            if (particlesModule != null && particlesModule.events().getValue("Тотем") && particlesModule.isEnabled()) {
