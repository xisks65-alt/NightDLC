...
import dev.wh1tew1ndows.baritone.api.event.events.ChatEvent;
...
import dev.wh1tew1ndows.baritone.utils.PlayerMovementInput;
...
import dev.wh1tew1ndows.client.managers.module.impl.movement.InvMove;
import dev.wh1tew1ndows.client.managers.module.impl.movement.Sprint;
...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    public void tick() {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ()))) {
            ...
            if (this.isPassenger()) {
                ...
                this.packetPitch = rotationPitch;
                this.packetYaw = rotationYaw;
                IBaritone baritone = BaritoneAPI.getProvider().getBaritoneForPlayer(this);
                if (baritone != null) baritone.getGameEventHandler().onPlayerUpdate(new PlayerUpdateEvent(EventState.PRE));
                this.onUpdateWalkingPlayer();
                if (baritone != null) baritone.getGameEventHandler().onPlayerUpdate(new PlayerUpdateEvent(EventState.POST));
            }
    ...
    private void onUpdateWalkingPlayer() {
        ...
        if (this.isCurrentViewEntity()) {
            ...
            if (motionEvent.isOnGround()) ++ticksOnGround;
    ...
    public void sendChatMessage(String message) {
        ChatEvent brEvent = new ChatEvent(message);
        IBaritone baritone = BaritoneAPI.getProvider().getBaritoneForPlayer(this);
        if (baritone == null) return;
        baritone.getGameEventHandler().onSendChatMessage(brEvent);
        if (brEvent.isCancelled()) return;
    ...
    public void livingTick() {
        ...
        boolean flag4 = (float) this.getFoodStats().getFoodLevel() > 6.0F || abilities.allowFlying || Sprint.getInstance().isEnabled() && Sprint.getInstance().antihynde.getValue();
        SprintLockEvent sprintLockEvent = new SprintLockEvent(false);
        sprintLockEvent.hook();
        ...
        if (Sprint.getInstance().isEnabled() && sprint) {
            if (!this.isSprinting() && (!this.isInWater() || this.canSwim()) && this.isUsingSwimmingAnimation() && flag4 && (!this.isPotionActive(Effects.BLINDNESS))) {
                this.setSprinting(true);
            }
        }
