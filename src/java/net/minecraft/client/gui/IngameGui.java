...
import dev.wh1tew1ndows.client.managers.events.other.ScoreBoardEvent;
import dev.wh1tew1ndows.client.managers.events.render.RenderCrosshairEvent;
...
import dev.wh1tew1ndows.client.managers.module.impl.render.Crosshair;
...
public class IngameGui extends AbstractGui {
    ...
    public void renderIngameGui(MatrixStack matrixStack, float partialTicks) {
        ...
        if (!this.mc.gameSettings.hideGUI) {
            ...
            if (!Crosshair.getInstance().isEnabled())
...
//            this.getFontRenderer().drawStringWithShadow(p_238447_1_, s, (float) (l1 - this.getFontRenderer().getStringWidth(s)), (float) k1, -1);
    ...
    private void func_238457_e_(MatrixStack p_238457_1_) {
        ...
        if (playerentity != null) {
            ...
            // Полоска насыщенности (дополнительная) - всегда отображается
            float saturationLevel = foodstats.getSaturationLevel();
            int saturationBars = MathHelper.ceil(saturationLevel / 2.0F);

            for (int s6 = 0; s6 < 10; ++s6) {
                int s7 = k1 - 10; // Позиция полоски насыщенности (выше полоски еды)
                int s8 = 16; // Базовый индекс текстуры
                int s9 = 0; // Смещение для эффектов

                // Проверяем, есть ли эффект голода
                if (playerentity.isPotionActive(Effects.HUNGER)) {
                    s8 += 36;
                    s9 = 13;
                }

                // Эффект дрожания при низкой насыщенности
                if (saturationLevel <= 0.0F && this.ticks % (l * 3 + 1) == 0) {
                    s7 = s7 + (this.rand.nextInt(3) - 1);
                }

                int s10 = j1 - s6 * 8 - 9;
                //     this.blit(p_238457_1_, s10, s7, 16 + s9 * 9, 27, 9, 9);

                // Рисуем заполненную часть полоски насыщенности
                if (s6 < saturationBars) {
                    this.blit(p_238457_1_, s10, s7, 16 + s9 * 9, 27, 9, 9);
                    this.blit(p_238457_1_, s10, s7, s8 + 36, 27, 9, 9);
                }

                if (s6 * 2 + 1 == saturationBars) {
                    this.blit(p_238457_1_, s10, s7, s8 + 45, 27, 9, 9);
                }
            }
