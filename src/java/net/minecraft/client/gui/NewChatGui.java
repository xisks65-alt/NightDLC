...
import dev.wh1tew1ndows.client.utils.render.font.normal.NormalFont;
...
public class NewChatGui extends AbstractGui {
        ...
        if (!this.func_238496_i_()) {
            ...
            if (k > 0) {
                ...
                for (int j1 = 0; j1 + this.scrollPos < this.drawnChatLines.size() && j1 < j; ++j1) {
                    ...
                    BetterMinecraft better = BetterMinecraft.getInstance();
                    boolean isBetter = better.isEnabled() && better.smoothChat.getValue();
                    if (isBetter) {
                    ...
                    if (this.mc.gameSettings.ofChatBackground != 3) {
                        if (BetterMinecraft.getInstance().isEnabled() && BetterMinecraft.getInstance().customChat.getValue()) {
                            //  NewChatGui.fill(p_238492_1_, -2, (int) (d6 - d3), l + 4, (int) d6, j2 << 24);
                        } else {
                            NewChatGui.fill(p_238492_1_, -2, (int) (d6 - d3), l + 4, (int) d6, j2 << 24);
                        }
                    }
                    if (better.smoothChat.getValue() && BetterMinecraft.getInstance().isEnabled()) {
                    ...
                    if (BetterMinecraft.getInstance().isEnabled() && BetterMinecraft.getInstance().customChat.getValue()) {
                        NormalFont.MEDIUM.get(18).draw(p_238492_1_, getStringFromChatLine(chatline), 0, (int) (d6 + d4), -1, false);
                        //  this.mc.fontRenderer.func_238407_a_(p_238492_1_, chatline.getLineString(), 0.0f, (int) (d6 + d4), 0xFFFFFF + (i2 << 24));
                        //   Fonts.SFX_SEMIBOLD.draw(p_238492_1_, getStringFromChatLine(chatline), 0, (int) (d6 + d4), -1, 9);
    ...
    private String getStringFromChatLine(ChatLine chatline) {
        IReorderingProcessor lineString = (IReorderingProcessor) chatline.getLineString();
        StringBuilder sb = new StringBuilder();

        ICharacterConsumer consumer = (index, style, codePoint) -> {
            net.minecraft.util.text.Color color = style.getColor();
            if (color != null) {
                int rgb = color.getColor();
                sb.append(String.format("\u00A7#%06X", (0xFFFFFF & rgb)));
            }
            sb.appendCodePoint(codePoint);
            return true;
        };

        lineString.accept(consumer);
        return sb.toString();
    }
