...
public class InventoryScreen extends DisplayEffectsScreen<PlayerContainer> implements IRecipeShownListener {
    ...
    protected void init() {
        ...
        if (this.minecraft.playerController.isInCreativeMode()) {
            ...
            this.addButton(new Button(width / 2 - 100 / 2, height - 30, 100, 20,
                    new StringTextComponent("Delete All"), (button) -> {
                if (mc.player != null && mc.playerController != null) {
                    Deleteall();
                }
            }));
            this.addButton(new Button(
                    width / 2 - 100 / 2, height / 2 - 110, 100, 20,
                    new StringTextComponent("Drop All"),
                    (button) -> {
                        if (mc.player != null && mc.playerController != null) {
                            dropItems();
                        }
            ...
            for (int i = 0; i < 36; i++) {
                ...
                if (stack != null && stack.getItem() != Items.AIR) {
                    ...
                    for (int attempt = 0; attempt < 3; attempt++) {
                        mc.playerController.windowClick(0, slotId, 45, ClickType.SWAP, mc.player);
                        try {
                            Thread.sleep(20);
    ...
    public void dropItems() {
        new Thread(() -> {
            try {
                for (int i = 0; i < this.container.getInventory().size() && mc.currentScreen == this; i++) {
