...
public class WinGameScreen extends Screen {
    ...
    private void drawWinGameScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        bufferbuilder.pos(0.0D, this.height, this.getBlitOffset()).tex(0.0F, f * 0.015625F).color(f3, f3, f3, 1.0F).endVertex();
        bufferbuilder.pos(i, this.height, this.getBlitOffset()).tex((float) i * 0.015625F, f * 0.015625F).color(f3, f3, f3, 1.0F).endVertex();
        bufferbuilder.pos(i, 0.0D, this.getBlitOffset()).tex((float) i * 0.015625F, f1 * 0.015625F).color(f3, f3, f3, 1.0F).endVertex();
        bufferbuilder.pos(0.0D, 0.0D, this.getBlitOffset()).tex(0.0F, f1 * 0.015625F).color(f3, f3, f3, 1.0F).endVertex();
        tessellator.draw();
    }
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        {
            this.blit(matrixStack, p_238665_2_, p_238665_3_, 0, 0, 155, 44);
        ...
        for (int i1 = 0; i1 < this.lines.size(); ++i1) {
            ...
            if ((float) l + f + 12.0F + 8.0F > 0.0F && (float) l + f < (float) this.height) {
                ...
                if (this.field_238664_v_.contains(i1)) {
                    this.font.drawStringWithShadow(matrixStack, ireorderingprocessor, (float) (j + (274 - this.font.func_243245_a(ireorderingprocessor)) / 2), (float) l, 16777215);
                } else {
                    this.font.random.setSeed((long) ((float) ((long) i1 * 4238972211L) + this.time / 4.0F));
                    this.font.drawStringWithShadow(matrixStack, ireorderingprocessor, (float) j, (float) l, 16777215);
                }
            }
        ...
        BufferBuilder bufferbuilder = tessellator.getBuffer();
        bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        bufferbuilder.pos(0.0D, k1, this.getBlitOffset()).tex(0.0F, 1.0F).color(1.0F, 1.0F, 1.0F, 1.0F).endVertex();
        bufferbuilder.pos(j1, k1, this.getBlitOffset()).tex(1.0F, 1.0F).color(1.0F, 1.0F, 1.0F, 1.0F).endVertex();
        bufferbuilder.pos(j1, 0.0D, this.getBlitOffset()).tex(1.0F, 0.0F).color(1.0F, 1.0F, 1.0F, 1.0F).endVertex();
        bufferbuilder.pos(0.0D, 0.0D, this.getBlitOffset()).tex(0.0F, 0.0F).color(1.0F, 1.0F, 1.0F, 1.0F).endVertex();
        tessellator.draw();
        RenderSystem.disableBlend();
