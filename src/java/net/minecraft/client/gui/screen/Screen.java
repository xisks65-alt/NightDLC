...
import dev.wh1tew1ndows.client.managers.events.other.ScreenOpenEvent;
...
public abstract class Screen extends FocusableGui implements IScreen, IRenderable {
    ...
    // Рисует текст по центру (X центрируется относительно указанной координаты)
    public void drawCenteredString(MatrixStack matrixStack, FontRenderer font, String text, int x, int y, int color) {
        font.drawString(matrixStack, text, (float) (x - font.getStringWidth(text) / 2), (float) y, color);
    }
    ...
    public void drawString(MatrixStack matrixStack, FontRenderer font, String text, int x, int y, int color) {
        font.drawString(matrixStack, text, (float) x, (float) y, color);
    }
    ...
    public void renderBackgroundCustom(MatrixStack matrixStack, float mouseX, float mouseY) {
        RenderUtil.Rounded.smooth(matrixStack, 0, 0, mc.getMainWindow().getScaledWidth(), mc.getMainWindow().getScaledHeight(), ColorUtil.multDark(InterFace.getInstance().clientColor(), 0.1F), Round.of(3));

        //  RenderUtil.drawShader1(mouseX, mouseY, Zetrix.inst().loadTime(), 0.75F);
    }
        ...
        if (this.minecraft.world != null) {
            if (BetterMinecraft.getInstance().blurInventory.getValue() && BetterMinecraft.getInstance().isEnabled()) {
                RenderUtil.Rounded.smooth(matrixStack, 0, 0, this.width, this.height,
                        ColorUtil.getColor(0, 0, 0, 135), Round.of(0));
