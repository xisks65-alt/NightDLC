...
public class ServerSelectionList extends ExtendedList<ServerSelectionList.Entry> {
    ...
    private void setList() {
        ...
        for (int i = 0; i < this.serverListInternet.size(); i++) {
            if (this.serverListInternet.get(i).getServer().isPinned()) {
                lastPinnedIndex = i;
            }
        }
        
        for (int i = 0; i < this.serverListInternet.size(); i++) {
            ServerSelectionList.NormalEntry entry = this.serverListInternet.get(i);
            this.addEntry(entry);
            
            if (i == lastPinnedIndex && lastPinnedIndex >= 0) {
                this.addEntry(new ServerSelectionList.SpacerEntry(25));
            }
        }
    ...
    public void updateOnlineServers(ServerList p_148195_1_) {
        ...
        java.util.List<ServerData> sortedServers = Lists.newArrayList();
        java.util.List<ServerData> pinnedServers = Lists.newArrayList();
        java.util.List<ServerData> normalServers = Lists.newArrayList();

        for (int i = 0; i < p_148195_1_.countServers(); ++i) {
            ServerData server = p_148195_1_.getServerData(i);
            if (server.isPinned()) {
        ...
        sortedServers.addAll(pinnedServers);
        sortedServers.addAll(normalServers);

        for (ServerData server : sortedServers) {
            this.serverListInternet.add(new ServerSelectionList.NormalEntry(this.owner, server));
        }
    ...
    protected int getRowTop(int index) {
        int baseTop = super.getRowTop(index);
        int extraHeight = 0;
        
        for (int i = 0; i < index && i < this.getEventListeners().size(); i++) {
            Entry entry = this.getEventListeners().get(i);
            if (entry instanceof SpacerEntry) {
                SpacerEntry spacer = (SpacerEntry) entry;
                extraHeight += spacer.getHeight() - this.itemHeight;
            }
        }
        
        return baseTop + extraHeight;
    }
    ...
    protected void renderList(MatrixStack p_238478_1_, int p_238478_2_, int p_238478_3_, int p_238478_4_, int p_238478_5_, float p_238478_6_) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if (entry instanceof SpacerEntry) {
                itemHeight = ((SpacerEntry) entry).getHeight();
            }
            int l = k + itemHeight;
    ...
    public static class SpacerEntry extends ServerSelectionList.Entry {
    ...
    public class NormalEntry extends ServerSelectionList.Entry {
        ...
        public void render(MatrixStack p_230432_1_, int p_230432_2_, int p_230432_3_, int p_230432_4_, int p_230432_5_, int p_230432_6_, int p_230432_7_, int p_230432_8_, boolean p_230432_9_, float p_230432_10_) {
            ...
            if (server.isAutodob()) {
                ...
                RenderUtil.Rounded.drawRoundedRect(p_230432_1_, p_230432_4_, p_230432_3_, 300, 32, 10, new Color(255,255,255).getRGB());
            }

            if (server.isPinned()) {
                RenderUtil.Rounded.roundedOutline(p_230432_1_, p_230432_4_ - 2, p_230432_3_ - 2, 304, 36, 2.5f, new Color(255, 163, 0, 255).getRGB(), Round.of(0));
            }
            ...
            if (this.mc.gameSettings.touchscreen || p_230432_9_) {
                ...
                if (server.isAutodob() || server.isPinned()) return;
            ...
            if (entry1 != null && entry2 != null) {
                if (entry1 instanceof NormalEntry normalEntry1 && entry2 instanceof NormalEntry normalEntry2) {
                    if (normalEntry1.server.isAutodob() || normalEntry2.server.isAutodob() || 
                        normalEntry1.server.isPinned() || normalEntry2.server.isPinned()) {
                        return;
                    }
                }
            }
        ...
        public boolean mouseClicked(double mouseX, double mouseY, int button) {
            ...
            if (d0 <= 32.0D) {
                ...
                if (!server.isAutodob() && !server.isPinned()) {
