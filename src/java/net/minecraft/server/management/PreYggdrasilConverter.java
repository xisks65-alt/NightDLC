...
public class PreYggdrasilConverter {
    ...
    public static UUID convertMobOwnerIfNeeded(final MinecraftServer server, String username) {
        if (!StringUtils.isNullOrEmpty(username) && username.length() <= 16) {
            ...
            if (gameprofile != null && gameprofile.getId() != null) {
                ...
                return PlayerEntity.getUUID(new GameProfile(null, username));
            }
        } else {
            try {
    ...
    private static boolean hasUnconvertableFiles() {
        boolean flag = OLD_PLAYERBAN_FILE.exists() && OLD_PLAYERBAN_FILE.isFile();

        boolean flag1 = OLD_IPBAN_FILE.exists() && OLD_IPBAN_FILE.isFile();

        boolean flag2 = OLD_OPS_FILE.exists() && OLD_OPS_FILE.isFile();

        boolean flag3 = OLD_WHITELIST_FILE.exists() && OLD_WHITELIST_FILE.isFile();
