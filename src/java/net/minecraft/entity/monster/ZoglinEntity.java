...
public class ZoglinEntity extends MonsterEntity implements IMob, IFlinging {
    ...
    public static AttributeModifierMap.MutableAttribute func_234339_m_() {
        return MonsterEntity.func_234295_eP_().createMutableAttribute(Attributes.MAX_HEALTH, 40.0D).createMutableAttribute(Attributes.MOVEMENT_SPEED, 0.3F).createMutableAttribute(Attributes.KNOCKBACK_RESISTANCE, 0.6F).createMutableAttribute(Attributes.ATTACK_KNOCKBACK, 1.0D).createMutableAttribute(Attributes.ATTACK_DAMAGE, 6.0D);
    }
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        ...
        if (this.world.isRemote) {
            return false;
        } else if (flag && source.getTrueSource() instanceof LivingEntity livingentity) {
