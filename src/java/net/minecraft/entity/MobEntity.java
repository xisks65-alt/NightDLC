...
public abstract class MobEntity extends LivingEntity {
    ...
    public MovementController getMoveHelper() {
        if (this.isPassenger() && this.getRidingEntity() instanceof MobEntity mobentity) {
    ...
    protected boolean shouldExchangeEquipment(ItemStack candidate, ItemStack existing) {
        if (existing.isEmpty()) {
            ...
            if (!(existing.getItem() instanceof SwordItem sworditem1)) {
                return true;
            } else {
            ...
            if (candidate.getItem() instanceof ToolItem) {
                ...
                if (existing.getItem() instanceof ToolItem toolitem1) {
    ...
    public boolean canAttack(LivingEntity target) {
        return (target.getType() != EntityType.PLAYER || !((PlayerEntity) target).abilities.disableDamage) && super.canAttack(target);
    }
