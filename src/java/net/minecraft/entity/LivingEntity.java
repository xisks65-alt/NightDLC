...
import dev.wh1tew1ndows.client.managers.events.player.DamageEvent;
...
import dev.wh1tew1ndows.client.managers.events.render.SwingAnimationEvent;
import dev.wh1tew1ndows.client.managers.module.impl.player.NoPush;
import dev.wh1tew1ndows.client.managers.module.impl.render.CustomHand;
...
import dev.wh1tew1ndows.client.utils.rotation.PlayerBox;
...
public abstract class LivingEntity extends Entity {
    ...
    public final StopWatch lastSwing = new StopWatch();
    ...
    public boolean onLivingFall(float distance, float damageMultiplier) {
        ...
        if (i > 0) {
            ...
            if (this instanceof ClientPlayerEntity) {
                DamageEvent damageEvent = new DamageEvent(DamageEvent.DamageType.FALL);
                damageEvent.hook();
            }
            return true;
        } else {
    ...
    private int getArmSwingAnimationEnd() {
        int animation;
        CustomHand animation2 = CustomHand.getInstance();

        if (animation2.isEnabled() && this instanceof ClientPlayerEntity && !animation2.swingMode.is("Выключено") && animation2.auraCheck()) {
            return 25 - (animation2.animspeed.getValue().intValue() * 2);
        }
    ...
    public void setSprinting(boolean sprinting) {
        SetSprintEvent setSprintEvent = new SetSprintEvent(this, sprinting);
        setSprintEvent.hook();
        sprinting = setSprintEvent.isState();
    ...
    private RotationMoveEvent storedEvent;
    /**
    ...
    protected void jump() {
        ...
        if (this instanceof ClientPlayerEntity) {
            IBaritone baritone = BaritoneAPI.getProvider().getBaritoneForPlayer((ClientPlayerEntity) this);
            if (baritone != null) {
                this.storedEvent = new RotationMoveEvent(RotationMoveEvent.Type.JUMP, this.rotationYaw);
                baritone.getGameEventHandler().onPlayerRotationMove(this.storedEvent);
                yaw = Rotation.YAW;
            }
        }

        JumpEvent event = new JumpEvent(jumpMotion, rotationYaw);
        if (this instanceof ClientPlayerEntity) {
            event.hook();
    ...
    public boolean isEating() {
        return this.isHandActive() && this.getActiveItemStack() != null && this.getActiveItemStack().getItem().isFood();
    }
