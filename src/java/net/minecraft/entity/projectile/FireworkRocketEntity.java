        ...
        if (this.isAttachedToEntity()) {
            ...
            if (this.boostedEntity != null) {
                if (this.boostedEntity.isElytraFlying()) {
                    Vector3d rotation = this.boostedEntity.getLookVec();
                    Vector3d motion = this.boostedEntity.getMotion();

                    FireworkMotionEvent fireworkMotionEvent = new FireworkMotionEvent(boostedEntity, 1.5D, 0.5D);
                    fireworkMotionEvent.hook();

                    double factor1 = 0.1D;
                    double factor2 = fireworkMotionEvent.getSpeed();
                    double interpolationFactor = fireworkMotionEvent.getInterpolation();

                    double newX = rotation.x * factor1 + (rotation.x * factor2 - motion.x) * interpolationFactor;
                    double newY = rotation.y * factor1 + (rotation.y * factor2 - motion.y) * interpolationFactor;
                    double newZ = rotation.z * factor1 + (rotation.z * factor2 - motion.z) * interpolationFactor;

                    this.boostedEntity.setMotion(motion.add(newX, newY, newZ));
                }
