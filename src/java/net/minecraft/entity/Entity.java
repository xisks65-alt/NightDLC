...
import dev.wh1tew1ndows.client.managers.events.player.PostMoveEvent;
import dev.wh1tew1ndows.client.managers.events.player.SpeedFactorEvent;
import dev.wh1tew1ndows.client.managers.events.player.StrafeEvent;
import dev.wh1tew1ndows.client.managers.module.impl.misc.SeeInvisibles;
...
public abstract class Entity implements INameable, ICommandSource {
    ...
    public boolean validDistance(Entity entity, float distance, boolean smart) {
        return this.getDistance(entity) < distance;
    }
    ...
    public float getDistanceToCoord(double xIn, double yIn, double zIn) {
        float f = (float) (this.getPosX() - xIn);
        float f1 = (float) (this.getPosY() - yIn);
        float f2 = (float) (this.getPosZ() - zIn);
        return MathHelper.sqrt(f * f + f1 * f1 + f2 * f2);
    }

    public float getDistanceToVec(Vector3d vec) {
        float f = (float) (this.getPosX() - vec.x());
        float f1 = (float) (this.getPosY() - vec.y());
        float f2 = (float) (this.getPosZ() - vec.z());
        return MathHelper.sqrt(f * f + f1 * f1 + f2 * f2);
    }

    public boolean isMaterialInBB(Material materialIn) {
        ...
        for (int l3 = j2; l3 < k2; ++l3) {
            for (int i4 = l2; i4 < i3; ++i4) {
                for (int j4 = j3; j4 < k3; ++j4) {
                    ...
                    if (Minecraft.getInstance().world.getBlockState(blockpos$pooledmutableblockpos).getMaterial() == materialIn) {
                        return true;
                    }
                }
            }
    ...
    public void moveRelative(float friction, Vector3d relative) {
        ...
        if (this instanceof ClientPlayerEntity) {
            ...
            IBaritone baritone1 = BaritoneAPI.getProvider().getBaritoneForPlayer((ClientPlayerEntity) (Object) Minecraft.getInstance().player);
            ...
            if (baritone1.getMineProcess().isActive() || baritone1.getFollowProcess().isActive() || baritone1.getBuilderProcess().isActive() || baritone1.getFollowProcess().isActive() || baritone1.getExploreProcess().isActive() || baritone1.getFarmProcess().isActive() || baritone1.getCustomGoalProcess().isActive() || baritone1.getGetToBlockProcess().isActive()) {
                ...
                yaw = dev.wh1tew1ndows.baritone.api.utils.Rotation.YAW;
            }

            StrafeEvent strafe = new StrafeEvent(this.rotationYaw, friction, (float) relative.y, (float) relative.z);
            strafe.hook();
            if (strafe.isCancelled())
                return;
        }
