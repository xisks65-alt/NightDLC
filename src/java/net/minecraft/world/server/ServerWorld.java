...
public class ServerWorld extends World implements ISeedReader {
    ...
    public ServerWorld(MinecraftServer p_i241885_1_, Executor p_i241885_2_, SaveFormat.LevelSave p_i241885_3_, IServerWorldInfo p_i241885_4_, RegistryKey<World> p_i241885_5_, DimensionType p_i241885_6_, IChunkStatusListener p_i241885_7_, ChunkGenerator p_i241885_8_, boolean p_i241885_9_, long p_i241885_10_, List<ISpecialSpawner> p_i241885_12_, boolean p_i241885_13_) {
        super(p_i241885_4_, p_i241885_5_, p_i241885_6_, false, p_i241885_9_, p_i241885_10_);
    ...
    private void addPlayer(ServerPlayerEntity player) {
        ...
        if (entity != null) {
            LOGGER.warn("Force-added player with duplicate UUID {}", player.getUniqueID().toString());
            entity.detach();
    ...
    private boolean addEntity0(Entity entityIn) {
        if (entityIn.removed) {
            LOGGER.warn("Tried to add entity {} but it was marked as removed already", EntityType.getKey(entityIn.getType()));
            return false;
    ...
    public void onEntityRemoved(Entity entityIn) {
        ...
        if (entityIn instanceof ServerPlayerEntity serverplayerentity) {
    ...
    public void playEvent(@Nullable PlayerEntity player, int type, BlockPos pos, int data) {
        this.server.getPlayerList().sendToAllNearExcept(player, pos.getX(), pos.getY(), pos.getZ(), 64.0D, this.getDimensionKey(), new SPlaySoundEventPacket(type, pos, data, false));
    }
    ...
    private void sendQueuedBlockEvents() {
        while (!this.blockEventQueue.isEmpty()) {
            ...
            if (this.fireBlockEvent(blockeventdata)) {
                this.server.getPlayerList().sendToAllNearExcept(null, blockeventdata.getPosition().getX(), blockeventdata.getPosition().getY(), blockeventdata.getPosition().getZ(), 64.0D, this.getDimensionKey(), new SBlockActionPacket(blockeventdata.getPosition(), blockeventdata.getBlock(), blockeventdata.getEventID(), blockeventdata.getEventParameter()));
            }
        }
    }
    ...
    private boolean fireBlockEvent(BlockEventData event) {
        ...
        return blockstate.isIn(event.getBlock()) && blockstate.receiveBlockEvent(this, event.getPosition(), event.getEventID(), event.getEventParameter());
    }
