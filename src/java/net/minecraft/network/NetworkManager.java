...
public class NetworkManager extends SimpleChannelInboundHandler<IPacket<?>> {
    ...
    protected void channelRead0(ChannelHandlerContext context, IPacket<?> packet) {
        BaritoneAPI.getProvider().getAllBaritones().forEach(baritone -> {
            if (baritone.getPlayerContext().player() != null && baritone.getPlayerContext().player().connection.getNetworkManager() == this) {
                baritone.getGameEventHandler().onReceivePacket(new dev.wh1tew1ndows.baritone.api.event.events.PacketEvent(this, EventState.PRE, packet));
            }
        });
        if (this.channel.isOpen()) {
            try {
        ...
        BaritoneAPI.getProvider().getAllBaritones().forEach(baritone -> {
            if (baritone.getPlayerContext().player() != null && baritone.getPlayerContext().player().connection.getNetworkManager() == this) {
                baritone.getGameEventHandler().onReceivePacket(new dev.wh1tew1ndows.baritone.api.event.events.PacketEvent(this, EventState.POST, packet));
            }
        });
    }
    ...
    private void dispatchPacket(IPacket<?> inPacket, @Nullable GenericFutureListener<? extends Future<? super Void>> futureListeners) {
        BaritoneAPI.getProvider().getAllBaritones().forEach(baritone -> {
            if (baritone.getPlayerContext().player() != null && baritone.getPlayerContext().player().connection.getNetworkManager() == this) {
                baritone.getGameEventHandler().onSendPacket(new dev.wh1tew1ndows.baritone.api.event.events.PacketEvent(this, EventState.PRE, inPacket));
            }
        });
        ...
        BaritoneAPI.getProvider().getAllBaritones().forEach(baritone -> {
            if (baritone.getPlayerContext().player() != null && baritone.getPlayerContext().player().connection.getNetworkManager() == this) {
                baritone.getGameEventHandler().onSendPacket(new dev.wh1tew1ndows.baritone.api.event.events.PacketEvent(this, EventState.POST, inPacket));
            }
        });
    }
